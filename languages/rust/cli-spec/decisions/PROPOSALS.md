...
Proposals awaiting a decision. Each is a candidate record: when accepted it graduates to an
`ADR-<slug>.md` file in this directory (see [README](./README.md) for the format).
...
The `cli-design` shelf has no `decisions/` directory today. The cargo-CLI-only rule captured in
[ADR-cargo-cli-only-dependencies](./ADR-cargo-cli-only-dependencies.md) generalizes (`cargo add` / `uv add` / `go get` —
resolve-and-lock, never hand-edit pins); the language-agnostic checklist item was already added to
`cli-design/99-checklist.md`. Proposal: **if/when** `cli-design` adopts a `decisions/` convention, record
a language-agnostic dependency-hygiene ADR there. Rationale: this is a cross-language principle, but
this shelf must not create a `decisions/` directory in `cli-design` before that shelf decides to adopt
