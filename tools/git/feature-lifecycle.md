...
Every branch here is short-lived: it lives a day or two, carries one author's work, and is deleted
at the merge. The model it runs is
...
Start here when you already have uncommitted changes on the trunk and want to move them into their
own issue and work-clone.
...
6. Optionally open a draft pull request against the trunk
...
Commit and push changes in the work-clone. Rebase onto `master` as needed to stay current. See
[rebase-workflow](./rebase-workflow.md) for the rebasing strategy.
...
Both paths print the PR URL and exit. The PR lands by squash merge, so its title becomes the trunk's
commit message and is the line that must follow the commit convention.

### Cleanup

After the PR is merged on the forge:

1. Pull `master` in the main repo to pick up the merge
2. Remove the work-clone
3. Delete the local branch with `git branch -D` — after a squash merge its commits are not ancestors
   of the trunk, so `-d` refuses it. The forge auto-deletes the remote branch on merge
