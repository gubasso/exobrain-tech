...
serves, so it lives in its bucket beside the chapter that explains it (ADR-docs-vs-library-boundary unaffected). Three
...
Amended by [ADR-the-planning-method-moves-to-plan-xp](./ADR-the-planning-method-moves-to-plan-xp.md) — those artifacts left the
