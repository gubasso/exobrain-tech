# A lock-pinned public tool input is a tool dependency, not an external dependency

## Context and Problem Statement

ADR-the-planning-method-moves-to-plan-xp makes this repository a consumer of `plan-xp` through a `flake.nix` input tracking that
project's `develop` branch. ADR-self-containment bars a load-bearing dependency on a resource outside the
...
ADR-self-containment bars a load-bearing dependency on external knowledge, and on a resource that can change or
...
someone updates it deliberately. An unpinned input would be a violation of ADR-self-containment, so a lock entry
...
This amends ADR-self-containment. Its rule is unchanged; what changes is that the boundary between knowledge and
