# Add Deprecated status and the Amended-by convention to the ADR canon
...
Superseded by [ADR-a-record-is-not-annotated-as-it-ages](./ADR-a-record-is-not-annotated-as-it-ages.md)
— the `Deprecated` status survives; the `Amended by` annotation does not.
