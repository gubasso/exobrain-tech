...
Amended by [ADR-an-epic-is-a-field-and-a-document](./ADR-an-epic-is-a-field-and-a-document.md) — an entry may carry one
gated reference to an epic document.

Amended by [ADR-closed-is-an-append-only-log](./ADR-closed-is-an-append-only-log.md) — `closed.yml` is ordered by
