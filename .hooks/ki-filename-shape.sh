...
for f; do
  ...
  case "$b" in
    KI-*) ;;
    *) echo "FAIL known-issues:case-id-is-a-slug $f: no KI- prefix"; status=1; continue ;;
  esac
  case "$b" in
    *.md) ;;
    *) echo "FAIL known-issues:case-id-is-a-slug $f: not a markdown file"; status=1; continue ;;
  esac
  s=${b#KI-}
  s=${s%.md}
  case "$s" in
