...
for f; do
  ...
  case "$b" in
    ADR-*) ;;
    *) echo "FAIL decision-records:filename-is-a-slug $f: no ADR- prefix"; status=1; continue ;;
  esac
  case "$b" in
    *.md) ;;
    *) echo "FAIL decision-records:filename-is-a-slug $f: not a markdown file"; status=1; continue ;;
  esac
  s=${b#ADR-}
  s=${s%.md}
  case "$s" in
